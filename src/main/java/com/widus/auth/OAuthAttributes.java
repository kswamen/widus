...
public class OAuthAttributes {
	...
	private static OAuthAttributes ofKakao(String userNameAttributeName, Map<String, Object> attributes) {
		...
		return OAuthAttributes.builder().name((String) kakaoProfile.get("nickname"))
				...
				.attributes(kakaoAccount).nameAttributeKey(userNameAttributeName).build();
	}

	public User toEntity() {
		return User.builder().name(name).email(email).picture(picture).role(UserRole.USER).build();
	}
}